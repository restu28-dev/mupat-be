...
const halaman_daftar = db.halaman_daftar; // Pastikan nama model benar
...
exports.create = (req, res) => {
  upload(req, res, (err) => {
    ...
    console.log("data_",data_halaman_daftar)
    
    halaman_daftar.create({
      id: req.body.id,
      password: req.body.password,
      no_hp: req.body.no_hp,
      NIK: req.body.NIK,
...
exports.update = async (req, res) => {
  ...
  try {
    const num = await halaman_daftar.update(req.body, { where: { id: id } });
    num[0] === 1 ? res.send({
...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    const num = await halaman_daftar.destroy({ where: { id: id } });
    num === 1 ? res.send({
...
exports.readAll = async (req, res) => {
  try {
    const data = await halaman_daftar.findAll();
    res.send(data);
  } catch (err) {
    res.status(500).send({
...
exports.readById = async (req, res) => {
  const id = req.params.id;
  try {
    const data = await halaman_daftar.findByPk(id);
